...
async def forward_to_admin(message: Message, state: FSMContext):
    ...
    forwarded = await bot.send_message(
        ...
        f"💬 Текст: {message.text or 'Нет текста'}"
    )
...
async def admin_panel(message: Message):
    if message.from_user.id not in [ADMIN_CHAT_ID]:  # Можно добавить несколько админов
...
async def process_broadcast(message: Message, state: FSMContext):
    ...
    for user in users:
        try:
            ...
            sent += 1
            await asyncio.sleep(0.1)  # Чтобы не забанили
        except:
            pass
...
async def admin_reply_to_user(message: Message):
    if message.reply_to_message:
        # Пытаемся извлечь ID пользователя из ответа
        text = message.reply_to_message.text or message.reply_to_message.caption
        if text and "ID:" in text:
            try:
                user_id_line = text.split("ID:")[1].split("\n")[0].strip()
                user_id = int(user_id_line.replace("`", ""))
            ...
            # Если админ просто пишет в чат без ответа
            await message.reply("ℹ️ Чтобы ответить пользователю, нажми 'Ответить' на его сообщении.")
    else:
